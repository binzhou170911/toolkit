...
async function calculateHash(input: string, algorithm: string): Promise<string> {
  try {
...
async function calculateAllHashes(input: string): Promise<string> {
...
export const hashCalculatorTool: Tool = {
  id: 'hash-calculator',
  name: 'Hash 计算',
  description: '计算 MD5、SHA-1、SHA-256、SHA-512 哈希值',
  icon: '🔑',
  category: '加密',
  keywords: ['hash', 'md5', 'sha', 'sha1', 'sha256', 'sha512', '哈希', '摘要', '校验'],
  inputType: 'text',
  outputType: 'text',
  actions: [
    {
      id: 'all',
      name: '全部',
      execute: calculateAllHashes
    },
    {
      id: 'md5',
      name: 'MD5',
      execute: calculateMD5
    },
    {
      id: 'sha1',
      name: 'SHA-1',
      execute: (input) => calculateHash(input, 'SHA-1')
    },
    {
      id: 'sha256',
      name: 'SHA-256',
      execute: (input) => calculateHash(input, 'SHA-256')
    },
    {
      id: 'sha512',
      name: 'SHA-512',
      execute: (input) => calculateHash(input, 'SHA-512')
    }
  ]
}
